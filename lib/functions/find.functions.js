...
/**
   ...
   * ```
   */
export const find = ({ list_name, key, value, query_key, type, is_included }) => {
    return (() => {
        const response = asc_find(list_name, key, value.toString(), query_key !== null && query_key !== void 0 ? query_key : null, type !== null && type !== void 0 ? type : 0, is_included !== null && is_included !== void 0 ? is_included : 0);
        if (response)
            try {
...
export const find_index = ({ list_name, key, value, query_key, type }) => {
    return (() => {
        const response = asc_find_index(list_name, key, value.toString(), query_key ? query_key : null, type !== null && type !== void 0 ? type : 0);
        return response;
    })();
};
