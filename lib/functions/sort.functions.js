/**
   ...
   */
export const sort = (asc, list_name, sort_key, descending) => asc
    ? (() => {
        try {
            const { main } = asc;
            return JSON.parse(main.__getString(main.sort(main.__newString(list_name), main.__newString(sort_key), descending !== null && descending !== void 0 ? descending : 0)));
        }
        catch (err) {
            console.error(err);
        }
