...
/**
   ...
   * ```
   */
const find = ({ asc, list_name, key, value, query_key, type, is_included }) => {
    return asc
        ? (() => {
            const response = asc.__getString(asc.find(asc.__newString(list_name), asc.__newString(key), typeof value === 'string' ? asc.__newString(value) : value, query_key ? asc.__newString(query_key) : undefined, type !== null && type !== void 0 ? type : 0, is_included !== null && is_included !== void 0 ? is_included : 0));
            try {
...
exports.find = find;
const find_index = ({ asc, list_name, key, value, query_key, type }) => {
    return asc
        ? (() => {
